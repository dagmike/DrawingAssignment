...
Control::Control(const wchar_t* name, int x, int y, const wchar_t* iconFilename, bool shapeControl) : name(name), x(x), y(y), iconFilename(iconFilename), shapeControl(shapeControl)
{
	this->selected = false;
}
